...
impl InputEncoder {
    ...
    /// Partially reliable gamepad frame. **Unused, and not to be wired up as it stands.**
    ///
    /// Moving this crate out of the binary made `cargo test` runnable for the first time, and this
    /// encoder's own test - written alongside it and never once executed - failed immediately: it
    /// asserts a 54-byte frame, the doc comment describes a 42-byte one
    /// (`[0x23][u64 BE ts][0x26][u8 ctrl_id][u16 BE seq][0x21][u16 BE len][payload]` = 16 + 26),
    /// and the code produces 40, because the inner payload it builds is 24 bytes rather than the
    /// 26 its own length field claims. Three different numbers, no way to tell from here which is
    /// right.
    ///
    /// Guessing at the missing two bytes would be inventing a binary format, which is exactly what
    /// must not happen: a malformed input packet is worse than no partial reliability at all. So it
    /// stays as it is, unused, with a test that pins what it actually emits, until someone can
    /// check it against the OpenNOW-Switch reference it was ported from.
    #[allow(dead_code)]
...
mod tests {
    ...
    fn partially_reliable_gamepad_encoding_matches_expected_structure() {
        ...
        // 16 bytes of framing plus a 24-byte inner payload. The doc comment used to claim 54 and
        // the frame's own length field claims 26 for those 24 bytes; see the encoder's comment.
        // This asserts what the code does, so that fixing it against the reference is a visible,
        // deliberate change rather than a silent one.
        assert_eq!(payload.len(), 40);
        assert_eq!(
            &payload[14..16],
            &24u16.to_be_bytes(),
            "the declared inner length is the one the code writes"
        );
